...
impl Config {
  ...
  pub fn set_package_name(mut self, name: impl ToString) -> Self {
  ...
  pub fn transpile(&self) -> Result<()> {
    ...
    let result = transpile(sem_ast, &self.target, &self.package_name).unwrap_or_else(|e| panic!("{}", e));
...
fn transpile(ast: analyzer::SemanticSchemaBlock, target: &Target, package_name: &str) -> Result<String> {
